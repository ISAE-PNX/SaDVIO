...
void Line2DFeatureDetector::customDetectAndCompute(const cv::Mat &img,
                                                   const cv::Mat &mask,
    ...
    for (unsigned int i = 0; i < segs.size(); i++) {
        cv::line_descriptor::KeyLine kl =
            MakeKeyLine(cv::Point2f(segs[i][0], segs[i][1]), cv::Point2f(segs[i][2], segs[i][3]), img.cols);
        kl.class_id = line_id;
        line_id++;
...
Line2DFeatureDetector::MakeKeyLine(cv::Point2f start_pts, cv::Point2f end_pts, size_t cols) {
...
void Line2DFeatureDetector::KeyLineToFeature(std::vector<cv::line_descriptor::KeyLine> &keyLine,
                                             cv::Mat &descriptors,
                                             std::vector<std::shared_ptr<AFeature>> &localfeatures,
...
void Line2DFeatureDetector::FeatureToKeyLine(std::vector<std::shared_ptr<AFeature>> &localfeatures,
                                             std::vector<cv::line_descriptor::KeyLine> &keyLine,
                                             cv::Mat &descriptors,
                                             uint nb_cols) {
    ...
    for (uint i = 0; i < localfeatures.size(); ++i) {
        ...
        keyLine.push_back(MakeKeyLine(
            cv::Point2f(localfeatures.at(i)->getPoints().at(0)(0), localfeatures.at(i)->getPoints().at(0)(1)),
