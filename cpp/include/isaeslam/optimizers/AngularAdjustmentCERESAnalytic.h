...
/*!
 * @brief Angular error cost function for a point landmark in the sensor frame. Parameters are delta update of the frame
 * pose and the landmark position.
 *
 * This cost function uses bearing vector that is compared to the projection of the landmark on the unit sphere, via an
 * arbitrary tangent plane. This is inspired by "VINS-Mono: A Robust and Versatile Monocular Visual-Inertial State
  ...
  public:
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
        ...
        if (jacobians != NULL) {
            ...
            Eigen::MatrixXd J_e_lmk = Eigen::MatrixXd::Zero(2, 3);
            J_e_lmk += P.transpose() * (Eigen::Matrix3d::Identity() - b_s_lmk * b_s_lmk.transpose()) / t_s_lmk.norm();
    ...
    const Eigen::Vector3d _bearing_vector, _t_w_lmk;
    const Eigen::Affine3d _T_cam0_w, _T_cam0_cam0p, _T_cam_cam0;
    double _sigma;
};

/*!
 * @brief An angular cost function that depends on the depth of the landmark and the scale of the motion
 */
...
/*!
 * @brief An angular cost function that depends on the depth of the landmark, the current frame pose and the anchor
 * frame pose.
 */
...
/*!
 * @brief An angular cost function for line landmarks that depends on the delta update of the frame pose and the line
 * parameters.
 */
